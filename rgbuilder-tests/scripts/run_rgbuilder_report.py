#!/usr/bin/env python3
"""Run rgBuilder feature matrix against Tier 1 e-commerce test apps and publish reports.

Outputs (default: rgbuilder-reports/):
...
PROJECTS: list[ProjectConfig] = [
    ...
    ProjectConfig(
        ...
        language="C",
        exclude="build,cmake-build-debug,.rgbuilder",
...
def resolve_rgbuilder(explicit: str | None) -> Path:
    if explicit:
    ...
    # Embedded under rgctl/rgbuilder-tests/scripts → workspace root is parents[2]
    root = Path(__file__).resolve().parents[2]
    candidates = [
        root / "target" / "release" / "rgctl",
        root / "target" / "debug" / "rgctl",
        root / "rgBuilder" / "target" / "release" / "rgctl",
        root / "rgBuilder" / "target" / "debug" / "rgctl",
    ...
    sys.exit(
        "Could not find rgctl. Set RGBUILDER=/path/to/rgctl or pass --rgctl."
    )
...
def render_language_markdown(
    ...
    lines = [
        f"# rgBuilder Report — {project.language}",
        "",
        f"**App:** `{project.dir_name}` · **Run date:** {run_date}  ",
        f"**rgBuilder:** `{rgbuilder_path}`  ",
    ...
    lines += [
        ...
        f"- Policy: [`../../rgbuilder-policy.json`](../../rgbuilder-policy.json)",
        "",
        "## Deep analysis",
        "",
        "### Program slice",
...
def render_language_html(
    ...
    parts = [
        ...
        "<head>",
        '<meta charset="utf-8">',
        f"<title>rgBuilder Report — {html_esc(project.language)}</title>",
        f"<style>{HTML_STYLE}</style>",
        "</head>",
        "<body>",
        f"<h1>rgBuilder Report — {html_esc(project.language)}</h1>",
        f'<p class="meta"><strong>App:</strong> <code>{html_esc(project.dir_name)}</code><br>',
        f"<strong>Run date:</strong> {html_esc(run_date)}<br>",
        f"<strong>rgBuilder:</strong> <code>{html_esc(rgbuilder_path)}</code><br>",
...
def render_summary_markdown(
    ...
    projects = active_projects(results)
    lines = [
        "# rgBuilder Summary Report — Tier 1 E-commerce Test Apps",
        "",
        f"**Run date:** {run_date}  ",
        f"**rgBuilder:** `{rgbuilder_path}`  ",
...
def render_summary_html(
    ...
    body_parts = [
        ...
        "<head>",
        '<meta charset="utf-8">',
        "<title>rgBuilder Summary Report</title>",
        f"<style>{HTML_STYLE}</style>",
        "</head>",
        "<body>",
        "<h1>rgBuilder Summary Report — Tier 1 E-commerce Test Apps</h1>",
        ...
        "RGBUILDER=/path/to/rgctl ./scripts/run_rgbuilder_report.py --update-readmes</pre>",
        "</body></html>",
    ]
    return "\n".join(body_parts)
...
def render_reports_index(run_date: str, results: dict[str, Any] | None = None) -> str:
    ...
    return f"""# rgBuilder reports

Published output from [`scripts/run_rgbuilder_report.py`](../scripts/run_rgbuilder_report.py).
...
```bash
./scripts/run_rgbuilder_report.py --update-readmes
```
"""
...
def update_parent_readme(repo_root: Path, results: dict[str, Any], run_at: str) -> None:
    ...
    marker = "## rgBuilder analysis results"
    if marker in text:
    ...
    block = [
        ...
        marker,
        "",
        f"Summary: **[rgbuilder-reports/REPORT.md](rgbuilder-reports/REPORT.md)** · "
        f"[HTML](rgbuilder-reports/REPORT.html) (run {run_at})",
        "",
        "**Language reports:** "
        + " · ".join(
            f"[{p.language}](rgbuilder-reports/languages/{p.id}.md)"
            for p in active_projects(results)
        ),
        "",
    ...
    block += [
        "",
        "Per-project details: [`rgbuilder-reports/languages/`](rgbuilder-reports/languages/) · "
        "each `ecommerce-*/README.md` § **rgBuilder**.",
        "",
...
def update_project_readmes(repo_root: Path, results: dict[str, Any], run_at: str) -> None:
    for p in active_projects(results):
        ...
        if "## rgBuilder" in text:
            text = text.split("## rgBuilder")[0].rstrip() + "\n"
...
Raw: [`../rgbuilder-reports/{p.id}-summary.json`](../rgbuilder-reports/{p.id}-summary.json)
"""
...
def main() -> int:
    ...
    parser.add_argument(
        "--rgctl",
        help="Path to rgctl binary (default: RGBUILDER env, PATH, or ../rgBuilder/target/debug/rgctl)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=None,
        help="Report output directory (default: <repo>/rgbuilder-reports)",
    )
    parser.add_argument(
        "--repo-root",
        type=Path,
        default=None,
        help="rgbuilder-tests repo root (default: parent of scripts/)",
    )
    parser.add_argument(
        "--no-clean",
        action="store_true",
        help="Do not remove existing .rgbuilder caches before discover",
    )
    parser.add_argument(
        "--update-readmes",
        action="store_true",
    ...
    rgctl = resolve_rgbuilder(args.rgbuilder)
