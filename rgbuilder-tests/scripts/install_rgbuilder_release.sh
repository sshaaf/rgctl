...
#   RGBUILDER_INSTALL_DIR=/tmp/rgbuilder-bin ./scripts/install_rgbuilder_release.sh
#
...
INSTALL_DIR="${RGBUILDER_INSTALL_DIR:-$ROOT/.rgbuilder-bin}"
...
if [[ -z "$TARGET" ]]; then
  case "$(uname -s)-$(uname -m)" in
    ...
    *)
      echo "Unsupported platform: $(uname -s)-$(uname -m). Set RGBUILDER_TARGET explicitly." >&2
      exit 1
      ;;
  esac
fi
...
ASSET="rgbuilder-${VERSION}-${TARGET}.tar.gz"
