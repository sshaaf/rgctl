...
> **Historical.** This release shipped a background daemon and stdio MCP server. Both were **removed after v0.4.8** — see [unreleased](unreleased.md). Current workflow: in-repo `{repo}/.rgctl/` and `rgctl -f json` subprocesses ([AGENTS.md](../../AGENTS.md)).

**rgBuilder is now rgctl.** This release is the product rename plus a background **HTTP daemon**, explicit **`--no-daemon`** mode for in-repo artifacts and CI, and (since removed) stdio MCP for IDEs.
...
- **`rgctl serve --daemon`** — background HTTP on `0.0.0.0:8080` with per-repo URLs under `/{reponame}/`.
...
- #67 — rgctl rename, daemon, `discover --full`, integration tests
...
4. If you relied on in-repo **`.rgctl/`**, either use **`--no-daemon`** or point follow-up commands at the daemon cache path from discover JSON.
