...
fn checkout_heading<'a>(
    nodes: &'a [rgbuilder_graph::schema::Node],
) -> Option<&'a rgbuilder_graph::schema::Node> {
    nodes.iter().find(|n| {
...
fn extract_repository_symbols_have_body_text() {
    ...
    let config = rgbuilder_extraction::discovery::DiscoveryConfig {
        languages: Some(vec!["markdown".to_string()]),
        ..rgbuilder_extraction::discovery::DiscoveryConfig::default()
    };
...
fn full_repo_populate_graph_has_body_text() {
    ...
    let checkout = nodes
        .iter()
        .find(|n| {
            n.node_type == rgbuilder_graph::schema::NodeType::Module
                && n.name == "Checkout Flow"
                && n.get_property("kind") == Some("heading")
        })
...
fn fixture_discover_snapshot_preserves_body_text_in_place() {
    ...
    let registry = Arc::new(rgbuilder_languages::default_registry());
    let pipeline = rgbuilder_pipeline::ProcessingPipeline::with_config(
        registry,
        rgbuilder_pipeline::PipelineConfig {
            discovery: DiscoveryConfig {
                languages: Some(vec!["markdown".to_string(), "java".to_string()]),
                ..DiscoveryConfig::default()
            },
            ..rgbuilder_pipeline::PipelineConfig::default()
        },
    );
    pipeline
...
fn fixture_snapshot_roundtrip_preserves_body_text() {
    ...
    let mut graph = rgbuilder_graph::CodeGraph::new();
    graph.load(nodes, edges).expect("load graph");

    let tmp = tempfile::tempdir().expect("tempdir");
    let snapshot_path = tmp.path().join("graph.snapshot.bin");
    let prepared = rgbuilder_graph::snapshot::PreparedGraphSnapshot::from_backend(graph.backend())
        .expect("prepared");
    prepared
