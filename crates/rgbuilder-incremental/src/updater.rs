...
impl IncrementalUpdater {
    ...
    fn full_rebuild(
        ...
        std::fs::create_dir_all(repo_root.join(".rgbuilder"))?;
        ...
        if !edges_to_add.is_empty() {
            ...
            for (from, to, edge_type) in edges_to_add {
                if !backend.has_edge(from, to, edge_type) {
                    new_edges.push(rgbuilder_graph::schema::Edge::new(from, to, edge_type));
                    added += 1;
                }
            }
            backend.insert_edges_batch(new_edges)?;
...
/// Delete `.rgbuilder` artifacts that embed `graph_digest` so they cannot outlive a compact.
fn invalidate_digest_sidecars(repo_root: &Path) {
    let targets = [
        ...
        repo_root.join(".rgbuilder").join("analysis_results.bin"),
...
fn relation_type_to_edge(relation_type: rgbuilder_plugin_api::RelationType) -> EdgeType {
    use rgbuilder_plugin_api::RelationType;
    match relation_type {
...
mod tests {
    ...
    fn test_incremental_update() {
        ...
        let pipeline = ProcessingPipeline::new(Arc::new(rgbuilder_languages::default_registry()));
        ...
        let updater = IncrementalUpdater::new(Arc::new(rgbuilder_languages::default_registry()));
    ...
    fn compact_update_invalidates_digest_sidecars() {
        ...
        let analysis = root.join(".rgbuilder").join("analysis_results.bin");
