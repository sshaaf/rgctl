...
pub fn cfg_ready(repo: &Path) -> bool {
    rgctl_analysis::CfgPdgArchive::default_path(repo).is_file()
}
...
fn run_slice(repo: &Path, args: &CpgArgs) -> Result<Value> {
    ...
    let source = std::fs::read_to_string(&path).map_err(rgctl_error::Error::from)?;
...
fn resolve_function(
    ...
    let source = std::fs::read_to_string(&file).map_err(rgctl_error::Error::from)?;
    Ok((node, source))
}
