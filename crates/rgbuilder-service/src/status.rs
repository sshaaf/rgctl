...
/// Filename under `.rgbuilder/`.
pub const PIPELINE_STATUS_FILE: &str = "pipeline_status.json";

/// Exclusive lock filename under `.rgbuilder/`.
...
pub fn status_path(repo: &Path) -> PathBuf {
    rgbuilder_graph::paths::artifact_path(repo, PIPELINE_STATUS_FILE)
}

/// Path of the lock file for `repo`.
#[must_use]
pub fn lock_path(repo: &Path) -> PathBuf {
    rgbuilder_graph::paths::artifact_path(repo, PIPELINE_LOCK_FILE)
}
...
pub fn try_acquire_lock(repo: &Path) -> Result<PipelineLock> {
    let dir = rgbuilder_graph::paths::artifact_dir(repo);
...
pub fn refresh_ready_flags(status: &mut PipelineStatus, repo: &Path) {
    let dash = rgbuilder_graph::paths::artifact_path(repo, "dashboard/index.html");
    status.dashboard_ready = dash.is_file();
    status.cfg_ready = rgbuilder_analysis::CfgPdgArchive::default_path(repo).is_file();
    status.semantic_ready = rgbuilder_analysis::SemanticIndex::default_path(repo).is_file();
}
...
pub fn write_digest_marker(repo: &Path, relative: &str, digest: &str) -> Result<()> {
    let path = rgbuilder_graph::paths::artifact_path(repo, relative);
