...
pub fn run_search(graph: &CodeGraph, repo: &Path, args: &SearchArgs) -> Result<Value> {
    ...
    let analysis_path = rgbuilder_graph::paths::artifact_path(repo, "analysis_results.bin");
...
fn apply_scope_filter(hits: &mut Vec<rgbuilder_analysis::SemanticHit>, scope: SearchScope) {
    match scope {
