...
impl CodeGraph {
    ...
    ///
    /// Prefer [`rgctl_pipeline::code_graph_from_repository`] in workspace builds.
    #[deprecated(
        since = "0.1.0",
        note = "Use rgctl_pipeline::code_graph_from_repository from the workspace root crate"
    )]
    pub fn from_repository(root: &Path) -> Result<Self> {
        let _ = root;
        Err(Error::Other(
            "Use rgctl_pipeline::code_graph_from_repository from the workspace root crate"
                .into(),
        ))
    }
    ...
    /// Write the mmap-friendly binary snapshot under `.rgctl/`.
...
mod tests {
    ...
    #[test]
    #[ignore = "moved to rgctl_pipeline::code_graph_from_repository"]
