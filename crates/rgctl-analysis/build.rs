...
fn main() {
    ...
    if distilled_path.is_file() {
        ...
        println!("cargo:rustc-env=RGCTL_VOCAB_MODEL_ID=vocab-accumulate-v2");
        println!(
            "cargo:rustc-env=RGCTL_VOCAB_MATRIX={}",
            out_path.display()
        );
        return;
    }
    ...
    println!("cargo:rustc-env=RGCTL_VOCAB_MODEL_ID=vocab-accumulate-v1");
    println!(
        "cargo:rustc-env=RGCTL_VOCAB_MATRIX={}",
        out_path.display()
    );
