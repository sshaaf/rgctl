...
Answer **structural** questions from a pre-built code knowledge graph instead of reading whole files into context. rgBuilder builds a queryable graph of your codebase and provides both MCP tools (for IDE integration) and CLI commands (for agents and CI).

## When to Use This Skill

Use rgBuilder when the user asks:
...
If `.rgbuilder/` exists, prefer rgBuilder queries over reading files.

## MCP vs CLI Decision

rgBuilder offers two interfaces:
...
|------|---------|-------------|
| `rgbuilder_status` | Pipeline status, artifact readiness | Check if graph/CFG/semantic index is ready |
| `rgbuilder_query` | GQL queries + macros | Inventory, callers/callees, communities, relationships |
| `rgbuilder_search` | Semantic search | Natural-language "where is checkout flow?" |
| `rgbuilder_impact` | Blast-radius analysis | "What breaks if I change X?" |
| `rgbuilder_metrics` | PageRank, betweenness, communities | "What are the hotspots/bottlenecks?" |
| `rgbuilder_cpg` | CFG/PDG/slice/mutations/flows | Data-flow, field writes, loop hazards |
| `rgbuilder_check` | Policy validation | CI gates, architectural rules |
...
**Critical:** Prefer `-f json` on **stdout**. Parse `schema_version` + payload from stdout. **Never use `2>/dev/null`** — it swallows rgBuilder errors and causes parse failures.
...
2. **Fast path (existing index)** — If `.rgbuilder/` exists **and** request is a structural question (not rebuild) → **do not re-run discover**. Route via workflow table; use MCP tools or CLI `-f json`
...
6. **Stop conditions** — Pure code-edit/debug with no structural need → do not force rgBuilder
...
| Check pipeline status | `rgbuilder_status` | `cpg status` |
...
| User Intent | MCP Tool | CLI Command |
|-------------|----------|-------------|
| Inventory functions | `rgbuilder_query` macro: `all_functions` | `gql --macro-name all_functions unused` |
| Find callers/callees | `rgbuilder_query` | `gql "MATCH (a)-[:CALLS]->(b) WHERE ..."` |
| Natural-language search | `rgbuilder_search` | `semantic query "checkout flow"` |
| List communities | `rgbuilder_query` macro: `all_communities` | `communities list` |
| Community members | `rgbuilder_query` | `gql "MATCH (f) WHERE f.community_id='12'"` |
| Subsystem ownership | `rgbuilder_search` scope: `community` | `semantic query "X" --scope community` |
...
| User Intent | MCP Tool | CLI Command |
|-------------|----------|-------------|
| Blast radius | `rgbuilder_impact` | `blast-radius <Symbol> --depth N` |
| Policy check (full codebase) | `rgbuilder_check` | `check --policy-file policy.json` |
| Policy check (one symbol) | `rgbuilder_impact` | `blast-radius <Symbol> --policy-file policy.json` |
...
| User Intent | MCP Tool | CLI Command |
|-------------|----------|-------------|
| PageRank hotspots | `rgbuilder_metrics` | `metrics --pagerank` |
| Betweenness bridges | `rgbuilder_metrics` | `metrics --betweenness` |
| Community stats | `rgbuilder_metrics` | `metrics --communities` |
...
| User Intent | MCP Tool | CLI Command |
|-------------|----------|-------------|
| CFG/PDG status | `rgbuilder_cpg` op: `status` | `cpg status` |
| Field mutations | `rgbuilder_cpg` op: `mutations` | `cpg mutations --type Foo` |
| Data flow trace | `rgbuilder_cpg` op: `flows` | `cpg flows FILE --line N --variable V --function F` |
| Program slice | `rgbuilder_cpg` op: `slice` | `slice FILE --line N --variable V --function F` |
| CFG inspection | `rgbuilder_cpg` op: `inspect` | `inspect <Symbol> cfg` |
| PDG inspection | `rgbuilder_cpg` op: `inspect` | `inspect <Symbol> pdg --edge-layer data` |
...
|-----------|--------------|
| "Generate migration plan / modernize" | `discover --with-cfg --with-security --export-migration-hints --migration-preset hybrid_default` then read `.rgbuilder/migration_plan.json` |
...
cat .rgbuilder/migration_plan.json
```
...
| Path | Content |
|------|---------|
| `.rgbuilder/graph.snapshot.bin` | Main graph snapshot |
| `.rgbuilder/semantic_index.bin` | Semantic index |
| `.rgbuilder/migration_plan.json` | Migration roadmap |
| `.rgbuilder/dashboard/` | Dashboard bundle |
| `.rgbuilder/analysis/` | CFG/PDG archives |
...
This writes `.claude/skills/rgbuilder/` and `.cursor/skills/rgbuilder/` from the embedded skill in the binary.
