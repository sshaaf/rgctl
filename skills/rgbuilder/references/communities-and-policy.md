...
The `communities` command reveals **implicit architecture** — functional clusters that rgBuilder automatically detects during `discover`. The Louvain algorithm partitions the call graph into groups of tightly connected functions, even if they span multiple files or packages.
...
- name: Policy Check
  run: |
    rgctl -r . discover .
    rgctl -r . check --policy-file .rgbuilder/policy.json
```
...
```
(Note: MCP `rgbuilder_impact` doesn't take `policy_file` arg — use `rgbuilder_check` for full policy)
