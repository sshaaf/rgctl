#!/usr/bin/env bash
# Python extraction-depth GQL + rgctl command verification.
# Fixture: rgctl-tests/ecommerce-python | Example: example/home-assistant (-l python)
set -euo pipefail
...
FIXTURE="${RGCTL_TESTS}/ecommerce-python"
EXAMPLE_REL="example/home-assistant"
...
run_fixture_gql() {
  ...
  assert_gql_min "method FQN (OrderService.*)" "MATCH (n:Function) WHERE n.qualified_name LIKE 'OrderService.*' RETURN n LIMIT 20" 1 "${FIXTURE}"
}
...
echo "=== python extraction GQL + commands ==="
run_fixture_gql
RGCTL_CMD_SKIP_DISCOVER=1 run_rgctl_commands_suite "${FIXTURE}"
run_example_smoke
echo "=== python extraction GQL + commands: OK ==="
