//! C++ extraction-depth GQL gates on `rgctl-tests/ecommerce-cpp`.
//!
//! ```bash
//! cargo build --release -p rgctl
//! cargo test --test cpp_langfeatures -- --nocapture
//! ```
...
fn repo() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("rgctl-tests/ecommerce-cpp")
}
...
#[test]
fn cpp_ecommerce_extends_nonzero() {
    ensure_discovered();
    let n = edge_count(&repo(), "EXTENDS");
    assert!(n >= 1, "expected Extends edges on ecommerce-cpp, got {n}");
}

#[test]
fn cpp_ecommerce_calls_gate() {
    ensure_discovered();
    let n = edge_count(&repo(), "CALLS");
    assert!(n >= 50, "expected >= 50 CALLS on ecommerce-cpp, got {n}");
}

#[test]
fn cpp_ecommerce_instantiates_nonzero() {
    ensure_discovered();
    let n = edge_count(&repo(), "INSTANTIATES");
    assert!(n > 0, "expected Instantiates edges on ecommerce-cpp, got {n}");
}
